  ...
  const html = `
    ...
    <section id="matrix" class="py-24 px-4 border-t border-slate-900 bg-slate-950/50" aria-labelledby="matrix-title">
      <div class="max-w-7xl mx-auto">
        ...
        <div class="mt-16 bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-xl text-slate-300">
          <h3 class="text-2xl font-bold text-white mb-6">Understanding the Example</h3>
          
          <div class="space-y-6">
            <div>
              <h4 class="text-lg font-semibold text-cyan-400 mb-2">The Microgrid Power Stabilization Task</h4>
              <p class="leading-relaxed">This example demonstrates a foundational computational task: initializing a 2x2 mathematical tensor representing a power grid's state, and multiplying it against a thermodynamic solver to calculate a stabilization delta.</p>
            </div>

            <div>
              <h4 class="text-lg font-semibold text-cyan-400 mb-2">Variables & Components</h4>
              <ul class="list-disc list-inside space-y-2 ml-2">
                <li><code class="text-rose-400 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800">solver</code>: A WebAssembly component instance allocated from the host environment. It exposes a mathematical engine to process thermodynamic vectors. Because it's an external resource, it <strong>must</strong> be manually freed to avoid memory leaks.</li>
                <li><code class="text-rose-400 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800">tensor</code>: A 2x2 numerical matrix <code class="text-slate-400">[1.2, 0.5, 0.9, 1.1]</code> representing the raw grid state (power and frequency metrics).</li>
                <li><code class="text-rose-400 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800">result</code> (or the return value): The calculated stabilization delta generated by multiplying the solver state by the grid tensor.</li>
              </ul>
            </div>

            <div>
              <h4 class="text-lg font-semibold text-cyan-400 mb-2">Constructing the IML Flat Arena</h4>
              <p class="leading-relaxed mb-4">Instead of relying on deep nesting, implicit memory models, or verbose identifiers, IML flattens the execution into a highly optimized JSON array called the <strong>Arena</strong>. Each object in the array represents a single operation, utilizing an ultra-terse schema:</p>
              <ul class="list-disc list-inside space-y-2 ml-2 mb-4">
                <li><code class="text-amber-400 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800">t</code> (Type): The operation type. For example, <code class="text-slate-400">{"L": null}</code> allocates a local resource, <code class="text-slate-400">{"B": [[...], 2, 2]}</code> initializes a Matrix, and <code class="text-slate-400">{"D": null}</code> Drops a resource.</li>
                <li><code class="text-amber-400 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800">c</code> (Children): An array of integer indices pointing to prior nodes in the Arena. This constructs the Directed Acyclic Graph (DAG) deterministically. For instance, <code class="text-slate-400">"c": [0, 1]</code> tells the Multiply node to use the outputs of Node 0 (the solver) and Node 1 (the tensor).</li>
                <li><code class="text-amber-400 bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800">r</code> (Rationale): An optional plain-English string mapped to the Semantic Overlay, enabling humans to easily debug the execution pipeline.</li>
              </ul>
              <p class="leading-relaxed">By compiling agent intent into this flat structure, IML enforces a <strong>Two-Pass Linear Type Check</strong>. If the <code class="text-slate-400">{"D": null}</code> (Drop) node is missing, the compiler statically guarantees failure before runtime, eliminating agent memory leaks entirely while saving massive amounts of context window tokens.</p>
            </div>
          </div>
        </div>
      </div>
